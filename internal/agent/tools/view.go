...
func NewViewTool(
	...
	return newReadFileTool(ViewToolName, visionClient, lspManager, permissions, filetracker, skillTracker, workingDir, skillsPaths...)
}

// NewReadTool creates a "read" tool that is functionally identical to
// "view". It exists as an alias so models that hallucinate a "read" tool
// (influenced by instructions like "read files" in the system prompt) can
// still read files.
func NewReadTool(
	lspManager *lsp.Manager,
	permissions permission.Service,
	...
	return newReadFileTool(ReadToolName, visionClient, lspManager, permissions, filetracker, skillTracker, workingDir, skillsPaths...)
}
		...
		func(ctx context.Context, params ViewParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			if isSupportedImage {
				...
				if !GetSupportsImagesFromContext(ctx) {
					// If a vision model is configured, route the
					// image through it and return the text
					// description to the calling model.
					if imageData, readErr := os.ReadFile(filePath); readErr == nil {
						if description, descErr := visionClient.DescribeImage(ctx, imageData, mimeType); descErr == nil && description != "" {
							filetracker.RecordRead(ctx, sessionID, filePath)
							return fantasy.NewTextResponse(
								"<system-reminder>\n" +
									"The following is a description of an image generated by a vision model. It may not be perfectly accurate. If the task can be solved without relying on visual details, prefer that approach.\n" +
									"</system-reminder>\n\n" +
									description,
							), nil
						}
